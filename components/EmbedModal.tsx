...
export default function EmbedModal({
  ...
  // Build the embed iframe source URL (defaults to Direct mode)
  const embedBase = origin ? `${origin}/embed` : '/embed';
  const targetEmbedSrc = `${embedBase}?src=${encodeURIComponent(videoUrl || 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4')}${videoTitle ? `&title=${encodeURIComponent(videoTitle)}` : ''}&mode=direct${autoplay ? '&autoplay=1' : ''}${muted ? '&muted=1' : ''}`;
  const directStreamUrl = videoUrl;
