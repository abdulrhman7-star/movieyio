...
import { Play, AlertCircle, Maximize, Volume2, VolumeX, Pause, RefreshCw, Settings, Check } from 'lucide-react';
...
function EmbedPlayerContent() {
  ...
  const paramMode = searchParams.get('mode') || 'direct';

  const [streamMode, setStreamMode] = useState<'direct' | 'proxy'>(paramMode === 'proxy' ? 'proxy' : 'direct');
  const [videoSrc, setVideoSrc] = useState<string>(rawSrc);
  const [qualities, setQualities] = useState<VideoQuality[]>(rawSrc ? [{ quality: 'HD', url: rawSrc }] : []);
  ...
  const toggleFullscreen = () => {
    const container = document.getElementById('embed-player-wrapper');
    if (!container) return;
  ...
  const handleQualityChange = (idx: number) => {
    if (!qualities[idx] || !videoRef.current) return;
    const currentTime = videoRef.current.currentTime;
    const wasPlaying = !videoRef.current.paused;

    setSelectedQualityIdx(idx);
    const newUrl = qualities[idx].url;
    setVideoSrc(newUrl);

    videoRef.current.src = streamMode === 'direct' ? newUrl : `/api/stream?url=${encodeURIComponent(newUrl)}`;
  ...
  const effectiveStreamUrl = videoSrc
    ? streamMode === 'direct'
      ? videoSrc
    ...
    >
      {loading ? (
        ...
        <>
          <video
            ...
            onError={() => {
              if (streamMode === 'direct') {
                ...
                setError('تعذر تشغيل الفيديو داخل المشغل المضمن');
              }
            }}
          >
          ...
          {/* Top Title Overlay when controls visible */}
          {title && showControls && (
            <div className="absolute top-0 inset-x-0 bg-gradient-to-b from-black/80 via-black/40 to-transparent p-4 flex items-center justify-between text-white transition-opacity pointer-events-none">
              <span className="text-sm md:text-base font-bold truncate drop-shadow-md">{title}</span>
            </div>
          )}

          {/* Quality Selector Overlay if multiple qualities exist */}
          {qualities.length > 1 && (
